...
if len(sys.argv) > 1:
    trial_seed = int(sys.argv[1])
else:
    trial_seed = 0

# DC current in nA
if len(sys.argv) > 2:
    DC_offset = float(sys.argv[2])
else:
    DC_offset = 0
...
# def syn_current_accumulation_callback(rec_dict):
#     rec_dict['ptr'].gather(rec_dict['buffer'])
#     syn_current_sum = np.sum(rec_dict['buffer'].as_numpy())
#     rec_dict['vec'].append(syn_current_sum)
...
def run_trial(simiter, run_sim=True):
    ...
    if mod_inh > 0:
        if mod_inh == 1:
            mod_inh_start = int(track_equilibrate / stim_dt)
            mod_inh_stop = mod_inh_start + int(inhibitory_manipulation_duration * input_field_duration / stim_dt)
        elif mod_inh == 2:
            mod_inh_start = int((track_equilibrate + modulated_field_center - 0.3 * input_field_duration) / stim_dt)
            mod_inh_stop = mod_inh_start + int(inhibitory_manipulation_duration * input_field_duration / stim_dt)
        elif mod_inh == 3:
            mod_inh_start = 0
        ...
        for group in stim_inh_syns:
            ...
            for syn in stim_inh_syns[group]:
                ...
                if mod_inh > 0 and group in inhibitory_manipulation_offset:
                    # inhibitory manipulation subtracts from the mean firing rate, but maintains the same theta
                    # modulation depth
            ...
            if excitatory_stochastic:
                ...
                for group in stim_exc_syns:
                    for syn in stim_exc_syns[group]:
                        f[str(simiter)]['successes'].create_dataset(str(index), compression='gzip',
...
sim = QuickSim(duration, cvode=0, dt=dt)
...
for sec_type in all_inh_syns:
    for i in local_random.sample(list(range(len(all_inh_syns[sec_type]))),
        ...
        GABA_syn_list.append(syn.target('GABA_A_KIN'))
...
for group in stim_exc_syns:
    for syn in stim_exc_syns[group]:
        ...
        AMPA_syn_list.append(syn.target('AMPA_KIN'))
