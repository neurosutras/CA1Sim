...
def main(mech_filename, synapses_seed, trial_seed, data_dir,
                        ...
                        for group in stim_exc_syns:
                            for syn in stim_exc_syns[group]:
                                f[str(simiter)]['successes'].create_dataset(str(index), compression='gzip', compression_opts=9,
                                            data=np.subtract(syn.netcon(AMPA_type).get_recordvec().to_python(),
    ...
    # TODO: stochastic inhibitory synapses not implemented yet
    ...
    AMPA_type = 'AMPA_KIN'
    NMDA_type = 'NMDA_KIN5'
    GABA_A_type = 'GABA_A_KIN'
    
    exc_syn_types = [AMPA_type, NMDA_type]
    inh_syn_types = [GABA_A_type]
