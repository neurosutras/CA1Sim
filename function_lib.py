...
def get_expected_vs_actual(expected_sim_file, actual_sim_file, sorted_sim_keys, interval=0.3):
    """
    ...
    single branch or path. The variable sorted_sim_keys contains the indexes of the simulations in the actual_sim_file
    corresponding to the branch or path, ordered by number of stimulated spines. This must be pre-computed.
    :param expected_sim_file: :class:'h5py.File'
    :param actual_sim_file: :class:'h5py.File'
    :param sorted_sim_keys: list of str
    :param interval: float
    :return: dict of list
    """
    equilibrate = actual_sim_file[sorted_sim_keys[0]].attrs['equilibrate']
    duration = actual_sim_file[sorted_sim_keys[0]].attrs['duration']
    actual = {}
    for sim in [actual_sim_file[key] for key in sorted_sim_keys]:
        t = sim['time'][:]
        interp_t = np.arange(0, duration, 0.01)
        left, right = time2index(interp_t, equilibrate-3., equilibrate-1.)
        start, stop = time2index(interp_t, equilibrate-2., duration)
