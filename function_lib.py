...
def get_expected_EPSP(sim_file, spine_index, equilibrate, duration):
    """
    ...
    """
    sim = (sim for sim in sim_file.itervalues() if sim.attrs['spine_index'] == spine_index).next()
    t = sim['time'][:]
...
def get_expected_vs_actual(actual_sim_list, expected_sim_file, interval=0.3):
    """
    Given an output file generated by parallel_expected_EPSP, and a list of outputs generated by filtering an output
    file generated by parallel_branch_cooperativity, this method returns a dict of lists, each containing an input-
    output function relating expected to actual peak depolarization for each recording site from stimulating a group of
    spines.
    :param actual_sim_list: list of :class:'h5py.Group'
    :param expected_sim_file: :class:'h5py.File'
    :param interval: float
    :return: dict of list
    """
    equilibrate = actual_sim_list[0].attrs['equilibrate']
    duration = actual_sim_list[0].attrs['duration']
    actual_sim_list.sort(key=lambda x: len(x.attrs['syn_indexes']))
    actual = {}
    for sim in actual_sim_list:
        t = sim['time'][:]
        interp_t = np.arange(0, duration, 0.01)
        left, right = time2index(interp_t, equilibrate-3., equilibrate-1.)
        start, stop = time2index(interp_t, equilibrate-2., duration)
    ...
    for i, spine_index in enumerate(spine_list):
        trace_dict = get_expected_EPSP(expected_sim_file, spine_index, equilibrate, duration)
        t = trace_dict['time']
