...
def build_kernel1(x, plot=False):
    ...
    if plot:
        ...
        axes.plot(filter_t[:len(local_filter)], local_filter / np.max(local_filter) * local_scale,
                  label='Local plasticity signal')
        axes.plot(filter_t[:len(global_filter)], global_filter / np.max(global_filter),
                  label='Global plasticity signal')
        axes.legend(loc='best', frameon=False, framealpha=0.5)
        axes.set_xlabel('Time (ms)')
        axes.set_ylabel('Kernel amplitude (a.u.)')
        clean_axes(axes)
        plt.show()
        plt.close()
...
def calculate_plasticity_signal(x, local_kernel, global_kernel, induction, plot=False):
    ...
    for i in range(len(induction_locs[induction])):
        ...
        for j, stim_force in enumerate(this_rate_maps):
            ...
            local_signal = np.convolve(0.001 * down_dt * this_stim_force, local_kernel_down)[:len(down_t)] / \
                           saturation_factor * local_scale * global_scale
            ...
            if plot and j == int(len(this_rate_maps)/2) and i == 0:
                fig, axes = plt.subplots(1)
                axes.plot(down_t, local_signal, label='Local signal')
                axes.plot(down_t, global_signal, label='Global signal')
    ...
    if plot:
        fig, axes = plt.subplots(1)
        axes.scatter(peak_locs['CA3'], plasticity_signal+1.)
...
def ramp_error1(x, xmin, xmax, ramp, induction=None, orig_weights=None, baseline=None, plot=False, full_output=False):
    ...
    if plot:
        ...
        axes.plot(x_bins, model_ramp, label='Model')
        axes.set_xlabel('Location (cm)')
...
# x0['1'] = [79.8939, 1497.7829, 0.6717, 17.0348, 292.1018, 0.0042]  # Err: 4.9940E+04
x0['1'] = [3.61E+01, 1.24E+03, 9.94E-01, 1.29E+01, 4.91E+02, 3.21E-03]  # Err: 1.929E+04, saturated
# x0['2'] = [231.3749, 2366.1465, 0.8026, 22.0357, 40.5840, 0.0143]  # Err: 1.1192E+05
x0['2'] = [4.96E+02, 2.34E+03, 6.00E-01, 1.00E+01, 2.55E+01, 1.96E-02]  # Err: 9.360E+04
# x0['3'] = [209.8419, 1279.7930, 0.5467, 22.9622, 61.9990, 0.0023]  # Err: 1.6808E+04
x0['3'] = [1.00E+01, 1.79E+03, 3.33E-01, 3.55E+01, 1.22E+02, 3.13E-03]  # Err: 9.785E+03
# x0['4'] = [449.6926, 2003.5177, 0.6553, 27.4732, 115.4437, 0.0013]  # Err: 8.4060E+04
x0['4'] = [1.00E+01, 2.13E+03, 2.24E-01, 1.83E+01, 5.00E+02, 1.46E-03]  # Err: 2.792E+04
# x0['5'] = [452.2207, 706.4445, 0.7317, 14.4542, 367.2086, 0.0016]  # Err: 2.9017E+04
x0['5'] = [3.59E+02, 6.97E+02, 3.65E-01, 2.07E+01, 3.76E+02, 2.40E-03]  # Err: 3.785E+03


# [local_rise_tau, local_decay_tau, local_scale, global_rise_tau, global_decay_tau, global_scale]
# x1 = [300., 1000., 0.05, 50., 500., 2.e-3]
if cell_id in x0:
    x1 = x0[cell_id]
else:
    x1 = x0['1']
xmin1 = [10., 500., 0.01, 10., 25., 1.e-3]
xmax1 = [500., 3000., 1., 50., 500., 5.e-2]
