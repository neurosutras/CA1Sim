...
def rinp_error(x, plot=0):
    ...
    interp_vm = np.interp(interp_t, np.array(sim.tvec), np.array(sim.rec_list[0]['vec']))
    left, right = time2index(interp_t, equilibrate-3., equilibrate-1.)
    result['v_rest'] = np.mean(interp_vm[left:right])
    Err = 0.
    for target in result:
        Err += ((target_val[target] - result[target])/target_range[target])**2.
    print('Simulation took %.3f s' % (time.time()-start_time))
    print('g_pas: %.4E, e_pas: %.3f, Error: %.4E, R_Inp: soma: %i, V_rest: %.3f' % (x[0], -x[1], Err, result['soma'],
                                                                                    result['v_rest']))
    if plot:
        sim.plot()
    else:
...
target_val = {'soma': 110., 'v_rest': -77.}
target_range = {'soma': 1., 'v_rest': 1.}

#the initial guess and bounds
# x = [soma.g_pas, e_pas]
x0 = [1e-5, 77.]
xmin = [1e-7, 60.]  # first-pass bounds
xmax = [1e-4, 90.]
#x1 = [  3.36e-05,   74.8]  # following first pass basinhopping
x1 = [  3.35e-05,   74.8]  # following polishing by simplex
