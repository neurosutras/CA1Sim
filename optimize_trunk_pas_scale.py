...
Magee 1995, Bittner et al., 2012 in the absence of Ih. Implements a linear gradient of g_pas along trunk, which is
...
def rinp_error(x, plot=0):
    ...
    cell.modify_mech_param('trunk', 'pas', 'g', origin='soma', slope=x[1])
    ...
    print('soma g_pas: %.4E, trunk slope: %.4E, Error: %.4E, soma R_inp: %.3f, '
          'V_rest: %.3f, trunk R_inp: %.3f' % (x[0], x[1], Err, result['soma'], v_rest, result['trunk']))
    if plot:
        sim.plot()
    else:
        return Err
...
# x = [soma.g_pas, trunk.g_pas slope]
x0 = [1.79e-5, 3.04e-7]
