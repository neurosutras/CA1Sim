...
def split_simulation(piece_index):
    ...
    for branch in cell.basal+cell.trunk+cell.apical+cell.tuft:
        if len(branch.spines) < 10:
            for node in branch.spines:
                syn = Synapse(cell, node, ['AMPA_KIN'], stochastic=0)
                syn_list.append(syn)
        else:
            for i in random.sample(range(0, len(branch.spines)), 10):
...
if __name__ == '__main__':
    ...
    print rec_file_list
    new_rec_filename = '030615 kap_kad_ih_ampar_scale kd no_na - EB2 - epsp_attenuation sample'
