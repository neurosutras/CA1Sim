...
def stim_actual_group((group_index, num_synapses)):
    """
    ...
    :return: str
    """
    synapse_group = groups_to_stim[group_index]
    if spines:
        loc = np.median([spine.synapses[0].loc for spine in spine_group['spines']]) #Need to fix this!!
    else:
        loc = np.median([synapse.loc for synapse in synapse_group['synapses']])
    path_type = synapse_group['path_type']
    path_index = synapse_group['path_index']
    ...
    synapse = synapse_group['synapses'][0]
    if spines:
        sim.modify_rec(2, node=synapse.parent.parent, loc=loc)
    else:
        sim.modify_rec(2, node=synapse.loc, loc=loc)
    if path_type == 'trunk':
