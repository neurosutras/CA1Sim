...
Current YAML file_path: data/parallel_optimize_GC_EPSC_attenuation_config.yaml
"""

context = Context()


def setup_module_from_file(param_file_path='data/parallel_optimize_GC_EPSC_attenuation_config.yaml', output_dir='data',
...
def setup_cell(verbose=False, cvode=False, daspk=False, **kwargs):
    ...
    candidate_branches = [apical for apical in cell.apical if
                          (100. < cell.get_distance_to_node(cell.tree.root, apical) < 150.) and apical.sec.L > 50.]
    ...
    if len(syn_list) < context.num_branches:
        print 'parallel_optimize_GC_EPSC_attenuation: cell with index %i: fewer than target number of branches meet ' \
