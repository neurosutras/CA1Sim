...
def spike_shape_features(x, export=False, plot=False):
    ...
    if prev_sim is 'spiking':
        print 'previous sim spiking'
        cell.reinit_mechanisms(reset_cable=True, from_file=True)
    else:
        print 'reinit cell'
        setup_spiking_optimize(spines, mech_file_path, neurotree_dict)
    sys.stdout.flush()
