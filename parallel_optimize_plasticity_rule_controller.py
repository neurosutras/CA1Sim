...
import parallel_optimize_plasticity_rule_engine
...
def build_kernel1(x, plot=False):
    ...
    if plot:
        ...
        axes.plot(filter_t[:len(local_filter)], local_filter * local_scale, label='Local plasticity signal')
        axes.plot(filter_t[:len(global_filter)], global_filter * global_scale, label='Global plasticity signal')
...
def calculate_plasticity_signal(x, local_kernel, global_kernel, induction, plot=False):
    ...
    """
    dv['x'] = x
    dv['local_kernel'] = local_kernel
    dv['global_kernel'] = global_kernel
    dv['induction'] = induction

    map_result = v.map_async(parallel_optimize_plasticity_rule_engine.calculate_plasticity_signal_piece,
                             range(len(induction_locs[induction])))
    last_line = ['' for i in range(len(map_result.stdout))]
    while not map_result.ready():
        time.sleep(1.)
        clear_output()
        for i, stdout in enumerate(map_result.stdout):
            if stdout:
                lines = stdout.split('\n')
                if lines[-2] and lines[-2] != last_line[i]:
                    last_line[i] = lines[-2]
                    print lines[-2]
        sys.stdout.flush()
    result = map_result.get()
    plasticity_signal = np.sum(result, axis=0)

    if plot:
        fig, axes = plt.subplots(1)
        axes.scatter(peak_locs['CA3'], plasticity_signal)
...
x1 = x0[cell_id]
xmin1 = [10., 500., 0.01, 10., 100., 1.e-3]
xmax1 = [300., 2000., 0.5, 100., 500., 5.e-2]
...
result = dv.execute('from parallel_optimize_plasticity_rule_engine import *')
while not result.ready():
    time.sleep(30)

dv['induction'] = 1
dv['peak_locs'] = peak_locs
dv['rate_maps'] = rate_maps
dv['induction_locs'] = induction_locs
dv['induction_durs'] = induction_durs
dv['extended_x'] = extended_x
dv['extended_t'] = extended_t
dv['dt'] = dt
dv['down_dt'] = down_dt
...
result = optimize_explore(x1, xmin1, xmax1, ramp_error1, ramp[induction], induction, maxfev=400)
