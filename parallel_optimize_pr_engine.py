...
class Pr(object):
    ...
    """
    def __init__(self, P0, f, tau_F, d1, tau_D1, d2=None, tau_D2=None):
        self.P0 = P0
        self.f = f
        self.tau_F = tau_F
        self.d1 = d1
    ...
    def stim(self, stim_time):
        ...
        if self.tlast is not None:
            ...
            self.D1 = 1. - (1. - self.D1) * np.exp(-(stim_time - self.tlast) / self.tau_D1)
            if self.d2 is not None:
                self.D2 = 1. - (1. - self.D2) * np.exp(-(stim_time - self.tlast) / self.tau_D2)
                self.P = min(1., self.P0 * self.F * self.D1 * self.D2)
            else:
                self.P = min(1., self.P0 * self.F * self.D1)
...
syn_list = SynList(cell, start=None)
