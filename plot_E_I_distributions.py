...
for rec_filename, group in zip(['062716 - cell169 - simple0_shape1.0 - saved parameters',
                                '062716 - cell170 - simple1_shape2.0 - saved parameters',
                                '062716 - cell171 - simple2_shape0.5 - saved parameters',
                                '062716 - cell172 - simple3_shape1.0 - saved parameters'],
                               ['shape_inh1.0', 'shape_inh2.0', 'shape_inh0.5', 'density_shape_inh1.0']):
...
for group in ['shape_inh1.0', 'shape_inh2.0', 'shape_inh0.5', 'density_shape_inh1.0']:
    ...
    axes.plot(rec_t, mean_ramp[group]['modinh0'], color='k')
    axes.plot(rec_t, mean_ramp[group]['modinh3'], color='orange')
    axes.set_ylim(-70., -40.)
    ...
    if svg_title is not None:
        ...
        fig.savefig(data_dir + svg_title + ' - single_compartment - ramp - ' + group + '.svg', format='svg', transparent=True)
    plt.show()
    plt.close()
    ...
    # axes.plot(rec_t, np.subtract(mean_ramp[group]['modinh0'], baseline), color='k')
    # axes.plot(rec_t, np.subtract(mean_ramp[group]['modinh3'], baseline), color='orange')
    axes.plot(rec_t, mean_theta_envelope[group]['modinh0'], color='k')
    axes.plot(rec_t, mean_theta_envelope[group]['modinh3'], color='orange')
    axes.set_ylim(0., 5.)
    axes.set_ylabel('Theta Vm (mV)')
    axes.set_xlim(0., 7500.)
    axes.set_xlabel('Time (s)')
    axes.set_xticks([0., 1500., 3000., 4500., 6000., 7500.])
    axes.set_xticklabels([0, 1.5, 3, 4.5, 6, 7.5])
    ...
    if svg_title is not None:
        fig.set_size_inches(2.05, 1.40)
        # fig.set_size_inches(1.3198, 1.2169)
        fig.savefig(data_dir + svg_title + ' - single_compartment - theta - ' + group + '.svg', format='svg', transparent=True)
    plt.show()
    plt.close()
