...
def plot_BTSP_model_ramp_detail_8(file_path, cell_id, self_consistent=False, savefig_dir=None, show=True):
    """
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        if not isinstance(cell_id, int) or str(cell_id) not in f[exported_data_key]:
            raise TypeError('cell_id: %i not found at file_path: %s' % (cell_id, file_path))
