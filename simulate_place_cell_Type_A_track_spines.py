...
rec_filename = 'output'+datetime.datetime.today().strftime('%m%d%Y%H%M')+'-pid'+str(os.getpid())+'-seed'+\
               ...
               '-track_spines_'+str(mod_weights)+'_'+str(trial_seed)
...
def run_trial(simiter):
    ...
    for group in stim_inh_syns:
        for syn in stim_inh_syns[group]:
            ...
            index += 1
    # pick 10 random spines each in field and out of field, record vm and Pr, and vm in the branch
    out_of_field_spines = np.where((np.array(peak_locs['CA3']) >= 850.) & (np.array(peak_locs['CA3']) <= 950.))[0]
    out_of_field_spines = local_random.sample(out_of_field_spines, 10)
    in_field_spines = np.where((np.array(peak_locs['CA3']) >= 4450.) & (np.array(peak_locs['CA3']) <= 4550.))[0]
    in_field_spines = local_random.sample(in_field_spines, 10)
    for index in out_of_field_spines+in_field_spines:
        syn = stim_exc_syns['CA3'][index]
        sim.append_rec(cell, syn.node, description='spine_vm')
        sim.append_rec(cell, syn.node, object=syn.target('Pr'), param='_ref_P', description='Pr')
        sim.append_rec(cell, syn.node.parent.parent, loc=syn.loc, description='branch_vm')
    sim.run(v_init)
    with h5py.File(data_dir+rec_filename+'-working.hdf5', 'a') as f:
