...
Here each compartment has point excitatory and point inhibitory conductances, and na channels are off, to generate
predictions about various excitatory and inhibitory distributions in and out of field.
"""
...
def run_trial():
    ...
    for group in stim_exc_syns:
        for i, syn in enumerate(stim_exc_syns[group]):
            ...
            node_index = syn.node.parent.parent.index
            if node_index not in stim_forces['AMPA']:
                stim_forces['AMPA'][node_index] = np.zeros_like(stim_t)
                stim_forces['NMDA'][node_index] = np.zeros_like(stim_t)
            stim_forces['AMPA'][node_index] = np.add(stim_forces['AMPA'][node_index], stim_force * AMPA_Po *
                                                     cos_mod_weight[group][i] * syn.target('AMPA_KIN').gmax)
            stim_forces['NMDA'][node_index] = np.add(stim_forces['NMDA'][node_index], stim_force * NMDA_Po *
                                                     syn.target(NMDA_type).gmax)
    for group in stim_inh_syns:
        for syn in stim_inh_syns[group]:
            ...
            stim_force = np.multiply(cos_mod_inh, inhibitory_theta_force)
            ...
            if node_index not in stim_forces['GABA']:
                stim_forces['GABA'][node_index] = []
            stim_forces['GABA'][node_index] = np.add(stim_forces['GABA'][node_index], stim_force * GABA_Po *
                                                     syn.target('GABA_A_KIN').gmax)
...
cell2 = CA1_Pyr(morph_filename, mech_filename, full_spines=False)
cell2.zero_na()
...
sim.append_rec(cell2, cell2.tree.root, description='soma', loc=0.)
...
stim_forces = {{} for syn_type in ['AMPA', 'NMDA', 'GABA']}
run_trial()
