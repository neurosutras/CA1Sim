...
def run_trial(simiter):
    ...
    global_time_offset = global_phase_offset / 2. / np.pi * global_theta_cycle_duration
    with h5py.File(data_dir+rec_filename+'-working.hdf5', 'a') as f:
        ...
        f[str(simiter)].attrs['phase_offset'] = global_time_offset
    if mod_inh > 0:
        if mod_inh == 1:
            ...
            mod_inh_start = int((track_equilibrate + modulated_field_center - inhibitory_manipulation_duration *
                                 input_field_duration / 2.) / dt)
            mod_inh_stop = mod_inh_start + int(inhibitory_manipulation_duration * input_field_duration / dt)
        elif mod_inh == 3:
            mod_inh_start = 0
            mod_inh_stop = len(stim_t)
        mod_inh_phase_offset = (stim_t[mod_inh_start] % mod_inh_theta_cycle_duration) / \
                                     mod_inh_theta_cycle_duration * 2. * np.pi
        mod_inh_phase_offset += global_phase_offset * mod_inh_theta_cycle_duration / global_theta_cycle_duration
        mod_inh_time_offset = mod_inh_phase_offset * mod_inh_theta_cycle_duration / 2. / np.pi
        sim.parameters['mod_inh_time_offset'] = mod_inh_time_offset
    ...
    for group in stim_exc_syns:
        for i, syn in enumerate(stim_exc_syns[group]):
            ...
            # stim_forces['exc'].append(stim_force)
    ...
    for group in stim_inh_syns:
        ...
        if mod_inh > 0 and group in inhibitory_manipulation_offset:
            ...
            # depth
            # inhibitory manipulation increases oscillation frequency of interneurons and LFP
            mod_inh_theta_force = np.exp(inhibitory_theta_phase_tuning_factor[group] *
                                         np.cos(inhibitory_theta_phase_offset[group] - 2. * np.pi * stim_t /
                                                mod_inh_theta_cycle_duration + mod_inh_phase_offset))
            mod_inh_theta_force -= np.min(mod_inh_theta_force)
            mod_inh_theta_force /= np.max(mod_inh_theta_force)
            mod_inh_theta_force *= inhibitory_theta_modulation_depth[group]
            mod_inh_theta_force += 1. - inhibitory_theta_modulation_depth[group]
            mod_inh_theta_force *= inh_peak_rate
        ...
        for syn in stim_inh_syns[group]:
            ...
            if mod_inh > 0 and group in inhibitory_manipulation_offset:
                stim_force[mod_inh_start:mod_inh_stop] = mod_inh_theta_force[mod_inh_start:mod_inh_stop] * \
                                                         mod_inh_multiplier
            # stim_forces['inh'].append(stim_force)
            train = get_inhom_poisson_spike_times(stim_force, stim_t, dt=stim_dt, generator=local_random)
            syn.source.play(h.Vector(np.add(train, equilibrate + track_equilibrate)))
            with h5py.File(data_dir+rec_filename+'-working.hdf5', 'a') as f:
    ...
    with h5py.File(data_dir+rec_filename+'-working.hdf5', 'a') as f:
        ...
        f[str(simiter)].create_dataset('output', compression='gzip', compression_opts=9,
                                    data=np.subtract(cell.spike_detector.get_recordvec().to_python(),
                                                     equilibrate + track_equilibrate))
    # return global_time_offset, mod_inh_time_offset
...
mod_inh_theta_cycle_duration = 145.
...
inhibitory_manipulation_offset = {'perisomatic': 9., 'axo-axonic': 9., 'apical dendritic': 9.,
                                    'distal apical dendritic': 9., 'tuft feedforward': 9., 'tuft feedback': 9.}
inhibitory_manipulation_duration = 0.7  # Ratio of input_field_duration
...
sim.parameters['manipulated_theta_cycle_duration'] = mod_inh_theta_cycle_duration
...
"""
stim_forces = {'exc': [], 'inh': []}
global_time_offset, mod_inh_time_offset = run_trial(trial_seed)
stim_forces['all_exc'] = np.sum(stim_forces['exc'], axis=0)
stim_forces['all_inh'] = np.sum(stim_forces['inh'], axis=0)
"""
