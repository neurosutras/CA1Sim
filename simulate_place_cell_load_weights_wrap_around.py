...
# .hdf5 file contains weight_distributions fit to each cell in the experimental dataset
if len(sys.argv) > 1:
    rule_group = str(sys.argv[1])
else:
    rule_group = 'long'
if len(sys.argv) > 2:
...
if rule_group == 'long':
    synapses_seed = int(cell_id)
    trial_seed_offset = 50000
elif rule_group == 'long_spont':
    synapses_seed = 100 + int(cell_id)
    trial_seed_offset = 60000
elif rule_group == 'short':
    synapses_seed = 200 + int(cell_id)
    trial_seed_offset = 70000
elif rule_group == 'short_spont':
    synapses_seed = 300 + int(cell_id)
    trial_seed_offset = 80000

# weights_filename = '120316 induced field long rule weights'
weights_filename = '032017 discrete plasticity summary'

rec_filename = 'output'+datetime.datetime.today().strftime('%m%d%Y%H%M')+'-pid'+str(os.getpid())+'_'+rule_group+\
               '_cell'+cell_id+'_trial'+str(trial_seed)
...
sim.parameters['rule_group'] = rule_group
sim.parameters['cell_id'] = cell_id
...
with h5py.File(data_dir + weights_filename + '.hdf5', 'r') as f:
    loaded_weights = f[rule_group][cell_id]['weights_parametric'][:]
    loaded_peak_locs = f['peak_locs'][:]
...
for group in stim_exc_syns:
    ...
    weights[group] = np.interp(peak_locs[group], loaded_peak_locs, loaded_weights)
...
local_random.seed(trial_seed_offset + int(cell_id) + 100 * trial_seed)
