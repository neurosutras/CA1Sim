...
rec_filename = 'output'+datetime.datetime.today().strftime('%m%d%Y%H%M')+'-pid'+str(os.getpid())+'-seed'+\
               str(synapses_seed)+'-e'+str(num_exc_syns)+'-i'+str(num_inh_syns)+'-simple_density_mod_inh'+str(mod_inh)+\
...
for group in stim_exc_syns:
    ...
    peak_loc_choices[group] = local_np_random.choice(in_field_peak_locs, this_modulated_num_exc_syns,
...
for group in all_exc_syns:
    for i in range(min(int(total_modulated_num_exc_syns*fraction_exc_syns[group]), all_exc_syns[group])):
