...
svg_title = '030916 - cell73'
collapsed_peak_locs = []
for group in peak_locs:
    collapsed_peak_locs.extend(peak_locs[group])
bins, density, null_average = sliding_window(collapsed_peak_locs)
inheritance = density - np.min(density)
fig, axes = plt.subplots(1)
axes.scatter(bins, density, color='r', label='Presynaptic Input Density Distribution')
...
plt.close()
fig, axes = plt.subplots(1)
axes.scatter(bins, inheritance, color='r', label='Presynaptic Input Density Distribution')
axes.set_xlim(0., 7500.)
axes.set_ylim(0., 600.)
axes.set_xlabel('Time (ms)', fontsize=20)
axes.set_ylabel('Input Density (/s)', fontsize=20)
axes.legend(loc='best', frameon=False, framealpha=0.5, fontsize=20)
clean_axes(axes)
plt.savefig(data_dir+svg_title+' - inheritance density.svg', format='svg')
plt.show()
plt.close()