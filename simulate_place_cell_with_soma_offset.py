...
Executes a short simulation with a somatic current injection with specified amplitude.
"""
...
if len(sys.argv) > 1:
    i_inj_amp = float(sys.argv[1])
else:
    i_inj_amp = 0.
if len(sys.argv) > 2:
    ttx = bool(int(sys.argv[2]))
else:
    ttx = False
if len(sys.argv) > 3:
...
rec_filename = 'output'+datetime.datetime.today().strftime('%m%d%Y%H%M')+'-pid'+str(os.getpid())+\
               '_soma_offset{:.3f}'.format(i_inj_amp)+'_ttx'+str(int(ttx))+'_'+str(trial_seed)
...
global_theta_cycle_duration = 150.
...
# sim = QuickSim(duration*0.8, cvode=0, dt=0.5)
...
sim.append_stim(cell, cell.tree.root, loc=0., delay=0., dur=duration, amp=i_inj_amp)
