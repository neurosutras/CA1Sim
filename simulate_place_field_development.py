...
script_filename = 'simulate_place_field_development.py'

default_morph_filename = 'EB2-late-bifurcation.swc'

default_mech_filename = '043016 Type A - km2_NMDA_KIN5_Pr'
...
@click.command()
@click.option("--mech-filename", type=click.Path(file_okay=True, dir_okay=False), default=None)
...
def main(mech_filename, morph_filename, num_exc_syns, num_inh_syns, induction_seed, trial_seed, field_loc,
    ...
    if field_loc is None:
        if induction_loc is None:
            trial_type = 'pre_induction'
        else:
            trial_type = 'induction_loc_%.1f' % (induction_loc)
    else:
        if induction_loc is None:
            trial_type = 'field_loc_%.1f' % (field_loc)
        else:
            trial_type = 'field_loc_%.1f-induction_loc%.1f' % (field_loc, induction_loc)

    global rec_filename
    rec_filename = 'output'+datetime.datetime.today().strftime('%m%d%Y%H%M')+'-pid'+str(os.getpid())+'-seed'+\
                   str(synapses_seed)+'-e'+str(num_exc_syns)+'-i'+str(num_inh_syns)+'-'+trial_type+'-seed_'+\
                   str(trial_seed)
    ...
    global stim_exc_syns
    stim_exc_syns = {'CA3': [], 'ECIII': []}
    global stim_inh_syns
    ...
    if field_loc is not None:
        sim.parameters['field_loc'] = field_loc
    if induction_loc is not None:
        sim.parameters['induction_loc'] = induction_loc
    ...
    fraction_exc_syns = {sec_type: float(total_exc_syns[sec_type]) / float(np.sum(total_exc_syns.values()))
    ...
    total_inh_syns = {sec_type: len(all_inh_syns[sec_type]) for sec_type in ['soma', 'ais', 'basal', 'trunk', 'apical',
                                                                             'tuft']}
    fraction_inh_syns = {sec_type: float(total_inh_syns[sec_type]) / float(np.sum(total_inh_syns.values())) for sec_type
                         in
                         ['soma', 'ais', 'basal', 'trunk', 'apical', 'tuft']}
    num_inh_syns = min(num_inh_syns, int(np.sum(total_inh_syns.values())))

    for sec_type in all_inh_syns:
        for i in local_random.sample(range(len(all_inh_syns[sec_type])),
    ...
    for group in stim_exc_syns:
        if stim_exc_syns[group]:
            ...
            if field_loc is not None:
                field_center = track_length * field_loc
                peak_mod_weight = 2.5
                tuning_amp = (peak_mod_weight - 1.)
                start_loc = field_center - input_field_width * 1.2 / 2.
                end_loc = field_center + input_field_width * 1.2 / 2.
    ...
    if induction_loc is not None:
        ...
        induction_delay = equilibrate + track_equilibrate + t[induction_start_index]
