...
#        h('for (x) print (x), diam(x)', sec=node.sec)
    ...
    def load_mech_dict(self, mech_filename=None):
        ...
        if not mech_filename is None:
            if mech_filename.endswith('.yml') or mech_filename.endswith('.yaml'):
                try:
                    return read_from_yaml(data_dir+mech_filename)
                except:
                    # Fallback for paths that already include 'data/'
                    return read_from_yaml(mech_filename)
            else:
                # Fallback to Pickle for specification based on legacy files
