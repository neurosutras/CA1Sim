...
LFP = excitatory_peak_rate / 2. + (excitatory_peak_rate / 2.) * np.sin(2. * np.pi /
                            global_theta_cycle_duration * (stim_t - global_phase_offset))
sum_exc_forces = np.sum(exc_forces, axis=0)
sum_exc_forces -= np.min(sum_exc_forces)
sum_exc_forces /= np.max(sum_exc_forces)
sum_exc_forces *= excitatory_peak_rate
sum_tuft_forces = np.sum(tuft_forces, axis=0)
sum_tuft_forces -= np.min(sum_tuft_forces)
sum_tuft_forces /= np.max(sum_tuft_forces)
sum_tuft_forces *= excitatory_peak_rate
sum_inh_forces = np.sum(inh_forces, axis=0)
sum_inh_forces -= np.min(sum_inh_forces)
sum_inh_forces /= np.max(sum_inh_forces)
sum_inh_forces *= inhibitory_peak_rate['perisomatic']
"""